...
fn parse_all_games_details_from_data(data: serde_json::Value) -> Result<Vec<FullGameDetails>, Box<dyn Error>> {
...
pub fn extract_future_game_start_timestamps(all_games: &[FullGameDetails]) -> Result<Vec<u64>, Box<dyn Error>> {
    ...
    for game in all_games {
        if game.game_state == "FUT" && !game.start_time_utc.is_empty() {
            match DateTime::parse_from_rfc3339(&game.start_time_utc) {
                ...
                Err(e) => eprintln!("Warning: Could not parse timestamp '{}': {}", game.start_time_utc, e),
            }
        }
    }
...
pub fn print_game_scores_details(games: &[&FullGameDetails]) {
    ...
    for game in games {
        println!(
            "{}: {} - {}: {} (State: {})",
            game.home_team_abbrev, game.home_team_score, game.away_team_abbrev, game.away_team_score, game.game_state
        );
    }
}
